...
// start
	...
	$('.hint .icon').hover(
		function() {
			$(this).siblings('.hint-text').fadeIn();
		},
		function() {
			$(this).siblings('.hint-text').fadeOut();
		}
	);

	$(document).click(function(event) {
		if ($(event.target).closest('.hint').length) return;
		$('.hint').find('.hint-text').fadeOut();
	...
	$('.tabs-nav').on('click', 'li:not(.current)', function() {
		var $this = $(this),
	...
	$('.tabs-nav li.current').each(function() {
		var $this = $(this),
			$parent = $this.parents('.tabs'),
			$tabs = $parent.find('.tabs-list'),
			$tab = $tabs.find('.tab'),
	...
	$('.smi-slider a').width( $(window).width() / 4 );


	$('.smi-slider').carouFredSel({
		circular: false,
		infinite: false,
		responsive: true,
		items: {
	...
	$('.profile-container').height( $('.profile-rotation.current').height() + 12);
	...
	function closeBurger() {
		$('.dropdown-menu').animate({left: -388}, 500);
